...
- **Server-side YouTube search with a two-tier cache** — Searches proxy through `/api/search`, keeping the API key out of the browser bundle. Results are cached in MongoDB and served directly for 24h, so repeated queries don't burn the 10k-unit daily quota. Entries are then retained (14-day TTL index, under YouTube's 30-day cap) purely as a fallback: past 24h a live search runs first and refreshes the entry, and the aging copy is only served if YouTube is unreachable or the quota is spent — so an exhausted quota degrades to slightly-stale results instead of an error.
